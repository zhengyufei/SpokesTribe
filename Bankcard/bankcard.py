bankcardList = [
        ...
        },
        {
            ...
            'patterns': [
                {
                    'reg': '/^(402658|410062|468203|512425|524011|622580|622588|622598|622609|95555|621286|621483|621485|621486|621299)(\d{10}|\d{11})$/g',
                    'cardType': "DC"
                },
                {
                    'reg': '/^(690755)\d{9}$/g',
...
def get_bankname_by_bankcode(bankcode):
    for temp in bankcardList:
        if temp['bankCode'] == bankcode:
            return temp['bankName']
...
def get_card_type_name(type):
    if type in cardTypeMap.keys():
...
def verify_bankcard(num):
    if not luhn_check(num):
        return (False, '卡号不对')

    url = 'https://ccdcapi.alipay.com/validateAndCacheCardInfo.json?_input_charset=utf-8&cardNo=%s&cardBinCheck=true' %(num)
    response = requests.get(url)
    dic = json.loads(str(response.content, encoding = "utf-8"))

    if not dic['validated']:
        return (False, dic['messages'])

    if 'DC' != dic['cardType']:
        return (False, '不是储蓄卡')

    try:
        bank = Bank.objects.get(code=dic['bank'])
    except Bank.DoesNotExist:
        bank = Bank.objects.get(id=0)

    return (True, bank, get_card_type_name(dic['cardType']), dic['bank'])